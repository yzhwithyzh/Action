...
0. **⚠ 条目判定改为 0–3 四档评分（0.7.0，2026-08-05，随新版 Excel）**。
   ...
   - 领域百分比的定义从「判 dup 的条目数 ÷ 可评估条目数」改为
     「得分 ÷ 可评分满分」（0.7.x 写作「(可评分满分 − 得分) ÷ 可评分满分」，
     见下条 0.8.0）。**两者在只打两端分时完全等价**，
     所以 §9 之后的表 2 分箱、表 3 矩阵、`unclear` 剔出分母这些结论一个字都不用改。
   - `criteria/*.yaml` 的 `dup_when` / `diff_when` 就地改读为**两端锚点**，
     字段名未改（改名要动 34 条口径，收益只有好看）。中间两档暂只有一条通用口径
     （「差异会不会改变临床或方法学解读」），写在 `prompts.py` 里 34 条共用，
     **未经方法学专家评审**，是当前最该补的一块（见 §15 待定项）。
   - 条目编号 `9` 随新版 Excel 改为 `9a`。
   - 0.6.0 的历史结果 JSON 会在读取时自动折成两端分，重算百分比与当年逐位一致。

0b. **⚠ 评分方向翻转（0.8.0，2026-08-18，甲方定稿）**。
   0.7.0 照抄了 Excel 表 1 表头的「完全相同 0 分 … 完全不同 3 分」；甲方后来定稿为
   「分数即相似度」，故 0.8.0 起 **3 = 完全相同、0 = 完全不同，分越高越重复**，
   重复百分比随之由 `(满分 − 得分) ÷ 满分` 改为 `得分 ÷ 满分`。

   - **两处一起翻的净效果是 pct / level / overall_level 一格不动** ——
     同一次评估在 0.7.x 与 0.8.0 下结论完全相同，翻转只重新标注了分数这一层。
     所以本文档 §9 之后的表 2 / 表 3 仍旧一字未改。
   - `RATING_VERDICT`（3/2→dup，1/0→diff）在翻转下也**不变**：
     0↔3、1↔2 对调后恰好还落在原来那一侧。
   - `criteria/*.yaml` 的字段名仍是 `dup_when` / `diff_when`（说的是「重复时」/「不同时」，
     与分数方向无关），只是各自挂到了另一端的分数上，口径正文一字未动。
   - 历史数据：0.7.x 的结果 JSON 按 `engine_version` 自动翻转
     （`AssessmentResult._accept_legacy_rating`——判据只能是版本号，
     翻转前后 rating 取值域一模一样，单看一份结果无从分辨方向）；
     库里的存量走 `sql/013-action-srd-score-flip-pg.sql`。
   - **拿甲方那份 xlsx 核对本引擎会读反**，口径的唯一真源是 `schemas.py` 的 `RATING_LABEL_*`。
