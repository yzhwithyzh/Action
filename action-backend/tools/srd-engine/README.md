...
新版 Excel 表 1 给每个条目加了一列评分，引擎照搬这四档：

| 分 | 含义 | 说明 |
|---|---|---|
| 0 | 完全相同 | 满足该条目的「0 分锚点」，没有可观察的差异 |
| 1 | 部分相同 | 主体一致，差异**不会改变**临床或方法学解读 |
| 2 | 部分不同 | 存在**会改变**解读的实质差异，但仍有明确共同部分 |
| 3 | 完全不同 | 满足该条目的「3 分锚点」，没有实质共同点 |
| —  | 证据不足 | 引擎自己的第五态 `unclear`：不计分，也不进分母 |

- **分越低越重复**，与百分比方向相反，读数时注意别看反。
- 领域满分 = 3 × 条目数 → 领域1 /24、领域2 /18、领域3 /42、领域4 /18，合计 **/102**
  （`checklist.py` 里对这四个数有断言，改清单结构会立刻失败）。
- 领域重复百分比 =（可评分满分 − 得分）÷ 可评分满分 × 100，之后照旧走表 2 分箱、表 3 查表。
- **与 0.6.0 兼容**：老口径是「dup 条数 ÷ (dup + diff)」。全部条目只打 0 或 3 分时，
  新公式退化成完全相同的数值；0.6.0 的历史结果 JSON 读进来会自动折成 0/3 分
  （`ItemResult` 上的 `_accept_legacy_verdict`），重算百分比与当年一致。
- 0 分与 1 分折成旧的 `dup`、2 分与 3 分折成 `diff`，仅供展示与前端兼容，
  **不参与百分比计算** —— 否则 1 分与 0 分就没区别了。
- 1 分 / 2 分目前只有一条通用口径（差异会不会改变临床或方法学解读），写在
...
3. **失败或抽空的批次补跑一轮**（0.7.0）—— 有些 OpenAI 兼容端点在长输入下会
   「HTTP 200 + 合法结构 + 字段全空」，不报错也不重试。对一篇真实的系统综述来说
   整批 facet 全空不可能是真相，`is_empty_facets` 认出来后重跑一轮；两轮都空就记警告放弃。
   不做这层的话，空壳会被当成正常结果**缓存下来**，之后每次判定都从空壳里读，整篇判 unclear。
...
| 症状 | 兜底 |
|---|---|
| `response_format.type: json_schema is not supported by this model`（400） | 自动降级到 `function_calling` → `json_mode`，成功的方式**粘住**，不再每次白撞一次 |
