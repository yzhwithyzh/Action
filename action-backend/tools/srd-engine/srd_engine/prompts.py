...
评分只有四档（分越低越重复，注意方向别反了）：
- "0" 完全相同：两篇在该条目上没有可观察的差异，完全满足该条目「0 分锚点」的描述。
- "1" 部分相同：主体一致，只有细节差异，且这些差异**不会改变**临床解读或方法学解读
  （例如检索了同一批主流数据库，只是一篇多检了一个小型区域库）。
- "2" 部分不同：存在**会改变**临床解读或方法学解读的实质差异，但两篇仍有明确的共同部分，
  不到「完全不同」的地步（例如都限定成人但一篇只收慢性期、另一篇急慢性都收）。
- "3" 完全不同：完全满足该条目「3 分锚点」的描述，两篇在该条目上没有实质共同点。

另有一个非评分取值：
- "unclear" 证据不足：任一篇的相关信息缺失或含糊到无法比较。该条目不计分、也不进分母。

关键规则：
1. 严格按下面每个条目给出的「0 分锚点 / 3 分锚点」判断，中间两档就在这两个锚点之间取，
   判据只有一条：**差异会不会改变临床或方法学解读**。不要用自己另一套直觉标准。
2. **两篇都明确报告「未做某事」属于做法一致，打 0 分**；
   两篇都「没写清楚」则是 unclear。这两种情况完全不同，务必分清。
3. 不要为了显得谨慎就一律打 1 分或 2 分。够得上锚点就打 0 或 3，
...
def _criteria_block(code: str) -> str:
    ...
    1 分与 2 分不在这里给 —— 它们由系统提示词里的通用档位定义统一覆盖，
    34 个条目各抄一遍纯属浪费 token，也容易抄出互相打架的口径。
    """
    c = criteria_of(code)
    lines = [
        '【评分锚点】',
        f'0 分（完全相同）当：{c.get("dup_when", "").strip()}',
        f'3 分（完全不同）当：{c.get("diff_when", "").strip()}',
...
**不要因为前面的条目打了 0 分就倾向于后面也打 0 分** —— 这是批量判定唯一的已知风险
