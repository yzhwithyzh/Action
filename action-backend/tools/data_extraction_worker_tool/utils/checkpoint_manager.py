...
文件位置: backend/tools/data_extraction_worker_tool/utils/checkpoint_manager.py
...
class DataExtractionCheckpointManager:
    ...
    def __init__(self, session_id: str, checkpoint_dir: Optional[str] = None):
        ...
        logger.info(f"CheckpointManager 初始化成功: session_id={session_id}, dir={checkpoint_dir}")
    ...
    async def save_file_result(
        ...
        流程:
        1. 将 result_data 保存为 JSON（file_name.json）
        2. 更新 DB 该文件状态为 completed
    ...
    async def need_resume(self, db: AsyncSession) -> bool:
        ...
        if needs_resume:
            ...
            logger.info("✓ 无断点数据: session=%s, 将从头开始处理", self.session_id)
    ...
    def cleanup(self) -> None:
        """
        清理 JSON 断点数据（任务**真正完成**后调用）

        注意：
        - DB 记录通常会保留作为审计与统计，不在这里删除
        """
        try:
            self.json_manager.cleanup()
            logger.info(f"✓ 断点数据清理成功: session={self.session_id}")
        except Exception as e:
            logger.error(f"✗ 清理断点数据失败: session={self.session_id}, err={str(e)}")
