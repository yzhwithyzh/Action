"""checklist 校验 Worker Service 启动脚本：单进程异步 Worker。

    cd action-backend
    python start_checklist_worker.py --env dev

与 `python -m tools.checklist_worker_tool` 等价 —— 真正的服务实现在
`tools/checklist_worker_tool/worker_service/`，这里只是给运维一个不用记模块路径的入口
（与 `start_srd_worker.py` 保持同一种用法），并补上模块入口里没有的东西：

1. 把 `action-backend/` 塞进 `sys.path`：从任意工作目录 `python /path/to/start_checklist_worker.py`
...
`start_checklist_worker.sh` 按 `APP_ENV` 带上，默认同样是 `dev`，上生产库时改传 `prod`。
worker 要连库取 checklist 条目与 `ai_models` 模型池，所以这个参数必须和后端服务指同一套库。
"""
...
from tools.checklist_worker_tool.config.worker_config import CONFIG  # noqa: E402
from tools.checklist_worker_tool.worker_service.worker_service import ChecklistWorkerService  # noqa: E402
from tools.common.bootstrap import ensure_repo_root, setup_logging  # noqa: E402

logger = logging.getLogger('checklist_worker')


def install_signal_fallback(service: ChecklistWorkerService) -> None:
...
async def main() -> int:
    ...
    logger.info('=' * 60)
    logger.info('报告规范校验 Worker Service 启动中...')
    logger.info('=' * 60)
    ...
    try:
        ...
        logger.info('=' * 60)
        logger.info('报告规范校验 Worker Service 已完全停止')
        logger.info('=' * 60)
    return 0
