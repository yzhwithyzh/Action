...
class LoginService:
    ...
    async def authenticate_user(
        ...
        # 闸门一：后台登录只拒绝**明确是访客**的账号。官网访客（'01'）与 sys_user 共用
        # 一张表，不拦的话访客账号能直接登进后台。文案必须与下面的密码错误完全一致，
        # 否则「用户存在但类型不对」会成为探测访客账号的信号。
        #
        # 这里绝不能写成 `!= SYSTEM_USER_TYPE`：user_type 是三态字段，除了 '00'/'01'
        # 还有 NULL——AddUserModel.user_type 默认 None，后台前端不传该字段，
        # add_user_dao 的 SysUser(**model_dump()) 把 None 显式赋到实例属性上，
        # SQLAlchemy 因此写入显式 NULL 而不会回落到 server_default '00'。于是后台
        # 新建/注册/导入的所有管理员都是 NULL，`None != '00'` 为真 → 账号当场被砖化。
        if user[0].user_type == CommonConstant.GUEST_USER_TYPE:
            logger.warning(f'非系统用户{login_user.user_name}尝试登录后台，已拒绝')
    ...
    async def get_current_user(
        ...
        if query_user.get('user_basic_info') is None:
            logger.warning('用户token不合法')
            raise AuthException(data='', message='用户token不合法')
        # 闸门二：只信 payload 会被伪造/旧 token 绕过，所以这里以库中的 user_type 为准。
        # 访客 token 打后台接口在这里被拦下，不会走完下面的角色/菜单/岗位组装逻辑。
        # 同闸门一，只拒绝明确的 '01'：user_type 为 NULL 的既有管理员必须放行。
        if query_user.get('user_basic_info').user_type == CommonConstant.GUEST_USER_TYPE:
            logger.warning('非系统用户token访问后台接口，已拒绝')
            raise AuthException(data='', message='用户token不合法')
