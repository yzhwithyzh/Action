...
# 报告规范校验 Worker 停止脚本
# 用途：优雅停止 tools/checklist_worker_tool 常驻 worker
#
# SIGTERM 后 worker 不会立刻退：它先停止取新任务，再等在跑的校验自然结束
# （上限 = CHECKLIST_WORKER_SHUTDOWN_TIMEOUT，默认 300s）。所以这里默认等 330s ——
# 一次校验是几十次模型调用，等不到就 kill -9 等于把用户正在跑的任务从中间打断，
# 而稿件只在队列与进程里流转、不落库，打断了就真没了。
#
# 可用环境变量：
#   CHECKLIST_WORKER_STOP_WAIT  最长等待秒数（默认 330）
...
MAX_WAIT="${CHECKLIST_WORKER_STOP_WAIT:-330}"
...
if ! checklist_is_worker_pid "${WORKER_PID}"; then
    echo "❌ PID ${WORKER_PID} 不是 Checklist Worker（进程已退出、PID 被复用），拒绝 kill"
    echo "   清理残留 PID 文件..."
    rm -f "${PID_FILE}"
    exit 1
fi
...
echo "   worker 会先停止取新任务，再等在跑的校验收尾（最长 ${MAX_WAIT}s）"
