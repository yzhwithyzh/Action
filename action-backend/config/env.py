...
class MailSettings(BaseSettings):
    ...
    官网访客注册/登录的验证码邮件走阿里云 DirectMail。凭据一律从 .env 读取，
    不得硬编码；mail_enabled 为 False 时不外发邮件，验证码改写入日志供联调。
    """

    mail_enabled: bool = False
    mail_access_key_id: str = ''
    mail_access_key_secret: str = ''
    mail_region: str = 'cn-hangzhou'
...
class GetConfig:
    ...
    def parse_cli_args() -> None:
        ...
        if 'alembic' in sys.argv[0] or any('alembic' in arg for arg in sys.argv):
            ...
            ini_config.read(Path(__file__).resolve().parent.parent / 'alembic.ini', encoding='utf-8')
        ...
        # 加载配置（按项目根目录定位，避免工作目录不同导致读不到.env而回落到默认配置）
