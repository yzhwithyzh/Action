...
-- ----------------------------
-- 大小写不敏感的唯一性约束
-- MySQL 默认 collation（utf8mb4_0900_ai_ci）不区分大小写，PostgreSQL 区分。
-- 没有这些索引，Admin 与 admin 会被视为两个不同账号，唯一性校验形同虚设。
-- 使用部分索引（del_flag = '0'）以免软删除后同名标识无法重建。
-- ----------------------------
CREATE UNIQUE INDEX IF NOT EXISTS uk_sys_user_lower_user_name
    ON sys_user (lower(user_name)) WHERE del_flag = '0';
CREATE INDEX IF NOT EXISTS ix_sys_user_lower_email
    ON sys_user (lower(email)) WHERE del_flag = '0';
CREATE UNIQUE INDEX IF NOT EXISTS uk_sys_role_lower_role_key
    ON sys_role (lower(role_key)) WHERE del_flag = '0';
CREATE UNIQUE INDEX IF NOT EXISTS uk_sys_config_lower_config_key
    ON sys_config (lower(config_key));
...
-- varchar 重载：SQLAlchemy 会为绑定参数生成 $1::VARCHAR 显式转换，
-- 只有 int8 重载时传字符串会直接抛 UndefinedFunctionError（PostgreSQL 不会隐式回退到 int8）
CREATE OR REPLACE FUNCTION "find_in_set"(varchar, varchar)
    RETURNS "pg_catalog"."bool" AS $BODY$
BEGIN
    RETURN POSITION(',' || $1 || ',' IN ',' || $2 || ',') > 0;
END;
$BODY$
    LANGUAGE plpgsql VOLATILE
                     COST 100;
...
SELECT c.relname                                                                           AS table_name,
       ...
       btrim(
            ...
            ELSE
            CASE
                -- 传 a.atttypmod 而非 NULL：NULL 会丢弃类型修饰符，使 varchar(30) 退化为
                -- 无长度的 character varying，代码生成因此拿不到字段长度
                WHEN t.typtype = 'd'::"char" THEN format_type(t.typbasetype, a.atttypmod)
...
FROM pg_attribute a
    ...
    -- contype = 'p' 必须限定：PostgreSQL 18 起 NOT NULL 约束也写入 pg_constraint（contype='n'），
    -- 不过滤会让每个 NOT NULL 列多出一行，并把所有 NOT NULL 列误判为主键、令 is_required 恒为 '0'
         LEFT JOIN pg_constraint con
                   ON con.conrelid = c.oid AND con.contype = 'p' AND (a.attnum = ANY (con.conkey))
...
SELECT c.relname              AS table_name,
       ...
       -- pg_class 不保存建表时间，此处为占位值。用 LOCALTIMESTAMP 而非 CURRENT_TIMESTAMP：
       -- 后者是 timestamptz，代码生成按 create_time::date 过滤时结果会随会话时区跨天漂移
