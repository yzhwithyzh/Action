...
本文档用于说明 `action-backend` 当前已经落地的 CLI 用法。
...
`ruoyi` 命令必须在后端项目根目录执行，也就是 `action-backend` 目录。
...
```json
{
  ...
  "command": [
    "alembic",
    "-c",
    "/path/to/action-backend/alembic.ini",
    "upgrade",
    "head"
  ],
  "cwd": "/path/to/action-backend"
}
```
...
table_names:
  - demo_table
output_file: /path/to/action-backend/gen_code_demo_table.zip
```
